...
def pede_categoria(dados: list) -> str:
    ...
    categoria = input('Escreva uma categoria da qual deseja consultar: ').replace(
        " ", '_').lower()

    while categoria not in categorias:
        categoria = input(
            'Categoria inválida! Escreva uma categoria da qual deseja consultar: ').replace(" ", '_')
    return categoria
...
def produto_mais_barato(produtos: list) -> dict:
...
def menu(dados: list) -> None:
    ...
    while opcao != '0':
        ...
        if opcao == '1':
            ...
            for produto in produtos:
                print(
                    f'{rank}) Id: {produto["id"]}, Preço: {produto["preco"]}')
                rank += 1
